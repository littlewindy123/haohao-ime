...
func TestCandidateLengthMatchesAndroidClient(t *testing.T) {
	for _, character := range []string{"中", "𠀀"} {
		t.Run(character, func(t *testing.T) {
			handler, translator, _ := testHandler(t, 50, 900_000)
			valid := sendTranslation(handler, "install", []string{strings.Repeat(character, 32)}, "candidate")
			if valid.Code != http.StatusOK {
				t.Fatalf("32-character candidate response = %d %s", valid.Code, valid.Body.String())
			}
			invalid := sendTranslation(handler, "install", []string{strings.Repeat(character, 33)}, "candidate")
			if invalid.Code != http.StatusBadRequest || translator.calls.Load() != 1 {
				t.Fatalf("oversized response = %d, translator calls = %d", invalid.Code, translator.calls.Load())
			}
		})
	}
}
