...
`main` 同时保留自动打字首屏、整句/单词卡真机实录，以及新版可输入的 CSS 立体键帽展台（`showroom.js` + `showroom.css`）。展台使用真实按钮，不依赖 WebGL；支持十款皮肤、拼音示例、大小写、退格和选区编辑。下方记录为历史迭代，GPU 贴图释放测试仅适用于首屏的 `scene.js`。完整构建统一打包首屏、展台和视频脚本，不再用单独补丁代替整站构建。
