...
`showroom.js` / `showroom.css` 使用真实按钮和 CSS 透视、键帽厚度实现立体皮肤键盘，点击、触摸和键盘激活共享同一输入逻辑。展示动画不能替代输入处理。支持选区替换、按完整 Unicode 字符删除、大小写、中英文模式及空格选词；中文仍只提供已有三组示例，不伪造任意拼音转换。切换四种配色与六种角色皮肤保留输入，不写本地存储。皮肤图来自原线上资源，继续标注同人预览及尚未内置 App。

此区域不依赖 WebGL，减少动态效果仅关闭透视与过渡，不移除交互。手机点击键帽不强制聚焦文本框，避免唤起系统键盘。`showroom.test.mjs` 验证编辑边界，`showroom.browser-check.js` 通过真实点击验证全部字母和操作，并覆盖 320 / 390 / 768 / 1440px、浅深色、十款皮肤、触摸、禁用 WebGL 与减少动态效果。

线上 `20260908-keycaps-v2` 的首屏和视频比当前仓库中的版本更新。只修皮肤区域时，应复制当前公开发布目录，在副本上运行 `node website/patch-showroom.mjs --site <发布副本>`。该命令更新键盘脚本、独立样式、皮肤资源及其哈希和 gzip，不覆盖其余首屏、视频和 APK。验证副本后再切换线上指针；不能用仓库的较早整站产物覆盖线上其他更新。正常全量构建也已收录键盘模块及皮肤资源。

2026-09-08 已部署到 `releases/20260908-keyboard-interactive-v1`，上一版 `20260908-keycaps-v2` 保留可回滚。发布前逐项校验清单，并确认非键盘资源及 APK 与上一版字节一致；线上已验证点按输入、空格选词、退格与皮肤切换。
