...
import {normalizeKeyboardSkin,keyFaceColor,readableKeyInk} from './keyboard-model.mjs';
...
test('key legend palettes have AA contrast',()=>{
 ...
 for(const value of Object.values(STUDIO_SKINS)){const s=normalizeKeyboardSkin(value);for(const id of ['Q','LANG','ENTER']){const color=keyFaceColor(s,id),ink=readableKeyInk(s.ink,color);const [hi,lo]=[lum(color),lum(ink)].sort((a,b)=>b-a);assert.ok((hi+.05)/(lo+.05)>=4.5,`${s.name}/${id}`);}}
});
...
test('interactive studio remains usable without a GPU and reduced motion removes transitions', async () => {
 ...
 assert.match(js, /createKeyboardScene/);
 assert.match(js, /import\("\.\/vendor\/scene-3d/);
 assert.match(js, /onFallback/);
 assert.match(js, /onLayout/);
