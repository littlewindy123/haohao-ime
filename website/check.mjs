...
assert.equal((html.match(/data-skin-choice=/g) || []).length, 10);
...
assert.match(showroom, /IntersectionObserver/);
assert.match(showroom, /prefers-reduced-motion/);
assert.doesNotMatch(showroom, /localStorage|sessionStorage|fetch\(|XMLHttpRequest|setInterval\(/);
const textBytes = gzipSync(Buffer.from(html + css + script + model + controller + showroom + skinModel)).length;
...
assert.ok(textBytes < 22 * 1024, "含新增换肤展台的文本压缩预算超限");
