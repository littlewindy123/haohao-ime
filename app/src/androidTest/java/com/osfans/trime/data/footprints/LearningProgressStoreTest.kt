...
class LearningProgressStoreTest {
    private val noon get() = Calendar.getInstance().apply { set(2026, 8, 8, 12, 0, 0); set(Calendar.MILLISECOND, 0) }.timeInMillis
    private fun test(block: suspend (InputFootprintDatabase, WordLearningStore) -> Unit) = runBlocking {
        ...
        try { block(db, WordLearningStore(db)) } finally { db.close() }
    }
    ...
    private suspend fun answer(store: WordLearningStore, now: Long, rating: RecallRating = RecallRating.REMEMBERED): String {
        ...
        store.reveal(card.token); store.answer(card.token, rating, now)
        return card.token
    }
    @Test fun disabledPlanCollectsFactsButDoesNotCheckIn() = test { _, store ->
        seed(store, 1); store.startSession(false, now = noon); answer(store, noon)
        val data = store.progress.dashboard(noon)
        assertNull(data.task); assertEquals(1, data.today.fresh); assertEquals(0, data.checkins)
    }
    @Test fun frozenGoalsNewWordsAndSettingsDoNotIncreaseTarget() = test { _, store ->
        seed(store); store.saveSettings(true, 1, 10, false)
        ...
        answer(store, noon); assertEquals(1, store.progress.dashboard(noon).total)
    }
    @Test fun quickSessionsContributeAndDoubleTapIsIdempotent() = test { db, store ->
        seed(store, 1); store.saveSettings(true, 5, 10, false)
    ...
    @Test fun retryBlocksCheckInAndUndoRetractsIt() = test { _, store ->
        seed(store, 1); store.saveSettings(true, 5, 10, false); store.startSession(true, now = noon)
    ...
    @Test fun undoInitialFeedbackRemovesDayAndCompletion() = test { _, store ->
        seed(store, 1); store.saveSettings(true, 5, 10, false); store.startSession(true, now = noon)
        val token = answer(store, noon); store.undoAnswer(token)
        val data = store.progress.dashboard(noon)
        assertEquals(0, data.today.total); assertFalse(data.task!!.completed); assertTrue(data.ratings.isEmpty())
    }
    ...
    @Test fun removingOneOutstandingGoalPreservesEarnedCompletionAndReaddingDoesNotInflateGoal() = test { _, store ->
        seed(store, 2); store.saveSettings(true, 5, 10, false); store.startSession(true, now = noon)
        answer(store, noon)
    ...
    @Test fun midnightCarriesUnansweredCardsAndDoesNotCountSkippedWords() = test { _, store ->
        ...
        store.startSession(true, now = noon + LEARNING_DAY_MS); answer(store, noon + LEARNING_DAY_MS)
    ...
    @Test fun timezoneChangesDoNotRewriteEventDate() = test { db, store ->
        seed(store, 1); store.startSession(false, now = noon); val token = answer(store, noon)
    ...
    @Test fun zeroAvailableTasksDoNotCheckInAndLegacyDaysAreNotInventedEvents() = test { db, store ->
        store.saveSettings(true, 5, 10, false); store.startSession(true, now = noon)
        assertFalse(store.progress.dashboard(noon).task!!.completed)
    ...
    @Test fun versionFourMigrationPreservesWordsSessionsSentencesAndLegacyDays() = runBlocking {
        ...
        android.database.sqlite.SQLiteDatabase.openDatabase(file.absolutePath, null, 0).use {
            it.execSQL("DROP TABLE learning_tasks"); it.execSQL("DROP TABLE learning_events"); it.version = 4
        }
        val upgraded = Room.databaseBuilder(context, InputFootprintDatabase::class.java, file.absolutePath)
            .addMigrations(LEARNING_PROGRESS_MIGRATION).build()
        try {
            val after = WordLearningStore(upgraded)
            assertEquals(word, after.find("测试0", "hello")); assertEquals(session, after.session())
        ...
        } finally { upgraded.close(); android.database.sqlite.SQLiteDatabase.deleteDatabase(file) }
    }
}
