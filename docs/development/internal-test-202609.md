...
当时只授权生成和备份新签名，未授权迁移手机，因此保留旧应用；未经迁移授权不能通过自动卸载解决签名冲突。后续新包统一使用新签名。后来获准完成的日常版迁移与更新约定见下文。
...
### 日常手机最新版更新约定（20260925 起）

所有者明确要求每次完成手机版更新后直接安装最新版，手机只保留一个日常版本。后续交付默认使用固定新签名和 `com.osfans.trime.debug` 包名，先备份、核验包名/版本/证书，再 `adb install -r` 覆盖升级，保留词本、进度与设置；不再只交付隔离验证结果。隔离测试包用完移除，不残留多个桌面入口。不得重新生成签名或上传内测 APK 到公开渠道。

20260925 已在所有者授权下完成一次旧签名到固定新签名的迁移。安装前新备份的 116 个应用数据文件全部恢复并逐文件核对 SHA-256；启动后再次核对词汇、剪贴板、常用语三个数据库的全部表内容与完整性，结果一致。原有设置只出现运行进程 ID 的正常变化，并通过首页开启新的立体键帽。原内测翻译和语音配置与旧安装包一致，未延长内测期限。

该次迁移安装为 `3.3.12 / 20260925`，实际安装 APK 的 SHA-256 为 `079503798bc303b1ef7ee865af2c0435ed3f2017d62db711acfa7e33bdb9e4b4`，证书为上述固定新签名。原 APK、迁移备份和校验记录留在本机受限的内测目录用于恢复，不作为手机安装的旧版本，也不公开发布。

随后完成设置与弹窗整理，日常手机已直接覆盖到 `3.3.12 / 20260927`。最终安装 APK 的 SHA-256 为 `703ae3f29cf1bc070adb0eed8ed41bd56de1f7a26acc0e4b36f11719e6209c60`，固定签名与内测翻译、语音配置均核验一致。使用 `adb install -r`，首次安装时间未变；安装前后的 117 个应用数据文件逐一校验相同，启动后再次检查词本、剪贴板、常用语共 14 张表，全部行内容一致，偏好只有运行进程 ID 正常变化。手机仅保留 `com.osfans.trime.debug`，并设为默认输入法。

20260927 将通知管理移入隐私与数据，不再在每次进入首页时弹出授权提示。未授予通知权限的启动与重建回归通过，并在日常手机确认启动无此提示；不自动替用户开启通知权限。界面与兼容规则见[设置与弹窗整理](product-ui-20260922.md#设置与弹窗整理)。
