...
## 2026-09-12：核心输入与编辑回归

以下为基于 `2ec9ccb6` 的后续增量验证，不替代上方历史性能测量。此次手机未连接，使用 API 36、x86_64 的只读模拟器和独立 `.regression` 应用；数据均为合成文本。

### 已确认的问题与处理规则

- **输入队列结束**：父作用域在工作协程启动前取消时，协程体内的 `finally` 不会执行；处理中取消又可能留下仍开放的通道。旧实现的四个新增 JVM 用例均失败。现在通过工作协程结束回调先关闭通道，再清理待处理命令和等待者；取消或关闭后拒绝新输入，正常输入仍按顺序处理。此改动不自动重启已结束的输入会话。
- **按需读取命令参数**：以前一个 `%s` 参数也会求值四种取词方式，每次都读取选区及光标两侧文本。现在只求值被格式串引用的参数，同一参数重复引用只读取一次；取词仍按原优先级回退，已有上屏内容或预编辑内容足够时不读取编辑框。保留普通、位置、相对索引、转义及混合格式转换，补齐多行格式支持。明确要求读取光标前全文时仍返回完整内容，不截断用户文字。
- **替换选区**：旧代码在 `setComposingText` 前额外删除选区外一个 UTF-16 单元，模拟器已复现选区前表情被拆坏。现在由编辑框直接替换选区或预编辑区域；写入被拒绝时不更新本地预编辑状态，异常路径也在 `finally` 中结束批量编辑。行为依据 [Android BaseInputConnection 实现](https://github.com/aosp-mirror/platform_frameworks_base/blob/master/core/java/android/view/inputmethod/BaseInputConnection.java)。
- **局部文本坐标**：`ExtractedText` 的选区相对于提取片段，向编辑器写回光标时必须加 `startOffset`。旧代码在应保留第 103 个位置时跳到第 3 个位置。现在片段内查找、全文内定位；无标点时到达片段边界，连续跳转不会停在当前分隔符，编辑器拒绝定位时返回失败。参见 [Android ExtractedText 契约](https://developer.android.com/reference/android/view/inputmethod/ExtractedText)。
- **初始化期间的自动填充**：系统可能在首次部署尚未完成时请求内联建议，旧路径直接读取未初始化配色并导致进程崩溃。主题未就绪或初始化失败时返回空请求，主题正常就绪后保留原建议样式。

### 回归入口与验收边界

- JVM：`RimeInputPipelineTest` 验证取消、关闭、等待者释放、千键顺序和刷新背压；`ActiveTextTest` 验证取词优先级、编辑框读取次数、长内容和格式兼容。
- Android：`EditorConnectionTest` 直接调用输入服务并使用 Android 编辑缓冲区，逐字断言正反向选区替换、表情完整性、取消输入、拒绝写入、异常批次、局部文档定位和主题初始化前的自动填充。该套件应在系统键盘被选中、回归 IME 尚未启动的新测试进程中运行。
- 最终 JVM 回归 307 项通过，无失败或跳过；上述 Android 编辑回归 8 项通过。经典／立体键帽跨配色与窗口模式的真实触摸用例、长按删除用例通过。长按删除采样未发现待处理队列积压，松手后仅剩一个已在途操作完成。
- 首次部署时，连续输入测试未在既有 15 秒窗口内等到键盘，不能把它记为通过；初始化完成后单独重跑通过，66 个按键全部处理，观察到 62 个中间候选版本。启动准备耗时仍待优化，此处只证明正常初始化后的输入顺序和中间刷新，不是响应延迟基准。
- 回归 APK 与测试 APK 构建、修改文件的 Spotless 检查、22 份 Markdown 校验均通过；`lintRegression` 为 0 错误、215 条警告，总数与修改前一致。已切回模拟器系统键盘、清理两个回归包并关闭只读模拟器；未安装手机日常版、变更版本号或提交推送。
- 本轮没有调整词频或解码模型，没有测出新的首选命中率、响应延迟或与搜狗的对比结果。后续候选优化应沿用固定合成语料，分别记录首选／前五命中、错拼类型、长句响应与冷唤起；模拟器功能测试不能替代同设备真机对照。
