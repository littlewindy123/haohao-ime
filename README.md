...
| 换一种方式记住 | 英译中、中译英、拼写与混合回忆，先想再揭晓；拼写草稿、复习进度和撤销都可恢复 |
| 从一本词书开始 | 基础高频、四级与六级参考词汇，支持个人词本及 CSV／粘贴导入；跨词本共享学习进度 |
| 集中练习薄弱词 | 遗忘与拼错专项独立加练，不占每日额度；词句、分组和完整学习进度可手动备份 |
| 看见自己的积累 | 学习趋势、跨日回忆表现、未来复习安排与打卡印章，使用本机真实学习记录；[查看学习规则](docs/business/personal-words.md) |
...
- **我的词本会同步吗？** 词本和复习进度保存在本机，没有账号同步；可在「我的学习 → 备份与恢复」主动导出 JSON。卸载或清除应用数据会删除本机记录，升级前请先备份。
...
[项目文档](docs/README.md) · [参与贡献](CONTRIBUTING.md)
...
- 云翻译默认关闭，离线候选与中文提交不依赖网络。候选来源固定为三档：“关闭”只查本地、“开启”只查云端、“混合”仅在本地缺失时查云端；旧“在线补全”开启值迁移为“混合”。首次选择需要网络的档位时会明确提示“仅待翻译文字会发送至所选服务”。
- 仅内部 Debug 调试包可通过 `-PembedInternalCloudSecrets=true` 内置阿里云和百度低权限凭据；Release 和回归包始终使用空共享凭据。调试双云中阿里云主用，仅在网络、鉴权、限流、额度或上游故障时切换百度；非法内容和参数错误不得切换。内置双云有效时隐藏旧公共服务、个人密钥和自定义接口表单。
- “翻译输入”固定中译英并使用独立中文草稿，停顿 `800ms` 后异步翻译，回车立即触发；整次请求最多等待 `6s`。原文或译文只有在用户明确点击上屏时才写入当前 App，整句内容不保存历史。
- 云候选每次最多处理五个顶部可见项，并保持原候选顺序；结果允许不超过 4 个英文词、32 个 Unicode code point 的短词组，支持单词内部的连字符和撇号。全局服务故障进入 `30s` 冷却，不得把具体候选负缓存；只有成功响应但无法展示的译文本身可以进入短期负缓存。
- 内置双云不依赖 Android Keystore；旧个人密钥和自定义接口继续由 API 23 以上的 Android Keystore 与 AES-GCM 保护。密钥、原文和译文不得进入日志、异常文本或缓存键，敏感输入框始终禁止云请求。
- 仓库保留历史公共网关代码 `service/translation-gateway/`，本轮本地优先方案不部署或接入该网关；用户主动配置的第三方翻译由手机直连服务商。Android 默认构建中的公共服务地址保持为空。
...
- 足迹使用独立的 `haohao_vocabulary.db`，并存放在 Android 排除自动备份和设备迁移的应用目录中；最近输入最多保留 100 个去重词。历史表只保存词头和遇见信息，独立词本保存用户确认的英文、可选 IPA、来源及学习进度，不随历史淘汰。
- 收藏不自动加入学习；轻复习和每日计划共享进度，支持四种回忆模式和单步撤销。「薄弱词」提供独立加练，不改变每日额度和正式排程。Room 版本 8 保留完整升级路径，具体规则见[个人词本说明](docs/business/personal-words.md)。
- 「我的学习」可导出完整学习备份，或选择完整恢复、仅合并内容；备份不包含输入历史、剪贴板或密钥。清空最近输入会保留收藏和学习进度，清空全部需明确确认。发音由用户点击触发，联网前沿用现有授权，缺配置、过期和播放失败均提供对应提示。
